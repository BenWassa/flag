...
> Carried forward: the scope-launcher concept, the continent-row direct-Play family (§6), the Play terminology migration (§11), the accessibility and motion requirements (§12–13).
> Cut: the 24-region colour taxonomy and its eight palette gates (§7), the modal direction (§9), the region-list A/B/C prototyping matrix (§5). See the assessment section of the superseding brief for why.
