...
**Related:** #28 Middle East learning scope, #54 river removal
...
Africa-specific runtime/generation entry points should be generalised once by the global expansion foundation issue. Africa remains the regression fixture proving that the refactor changes no existing geography, gameplay or visual behaviour.
...
Zero-land-neighbour countries remain accurate empty adjacency records and remain excluded from standard Neighbours rounds under the established gameplay contract.
...
A domain may deliberately exclude individual Neighbours targets only when the canonical application model gives them zero land neighbours. Missing topology coverage is not an acceptable long-term substitute for complete geography.
...
Recommended sequence after the shared foundation:
