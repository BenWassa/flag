...
**Status:** scoped behind the shared global expansion foundation; Neighbours parity has one explicit prerequisite decision because several Oceania regions contain no countries with land neighbours.
...
`*` The current Neighbours contract excludes zero-land-neighbour countries from standard rounds. That works in Africa because other eligible targets remain, but it makes true region-level Neighbours practice impossible for Australia & New Zealand, Micronesia and Polynesia and nearly empty for Melanesia.

Because the product requirement is four-domain parity for every learner-facing region, this incompatibility must be resolved deliberately before #27 can be called complete. Do not fake parity by marking an empty Neighbours curriculum as complete and do not invent maritime neighbours.

## Neighbours prerequisite: zero-land-neighbour learning

A dedicated product/domain change should decide how Atlas teaches countries with zero direct land neighbours while preserving the definition of neighbour.
...
The exact implementation belongs in its own focused prerequisite issue. #27 must consume that resolved contract rather than embedding an Oceania-only exception.
...
- zero-land-neighbour Neighbours interaction after its prerequisite lands;
