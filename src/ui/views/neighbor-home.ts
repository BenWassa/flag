...
export function renderNeighborHome(
  ...
  if (!config) {
    return `<main class="page"><h1 tabindex="-1" data-autofocus>Neighbor scope unavailable</h1><button class="button" data-action="route-parent">Back</button></main>`;
  }
    ...
    <main class="page">
      <header class="topbar topbar--detail">
        ...
        <div class="screen-title">
          ...
          <span>Neighbors · ${isAfrica ? 'Continent' : 'Region'} · ${stats.total} standard targets</span>
        </div>
      </header>

      <section class="scope-overview" aria-label="${escapeHtml(config.scope.label)} neighbor learning status">
        ...
        <div class="study-actions">
          <button class="study-action study-action--primary" data-action="start-neighbor-learn">
            <strong>Learn</strong>
            <span>Name every land neighbor · immediate feedback</span>
          </button>
          <button class="study-action" data-action="start-neighbor-test">
            <strong>Test</strong>
            <span>Same task · results summarized at the end</span>
          </button>
        </div>
      </section>
    ...
    <section class="atlas-section" aria-labelledby="neighbor-countries-heading">
      ...
      <div class="mini-ledger">
        ${countryIds.map((countryId) => {
          ...
          if (neighborCount === 0) {
            return `<div class="mini-ledger__row"><span>${escapeHtml(country?.name ?? countryId)}</span><span class="status-text">No land neighbors · excluded</span></div>`;
          }
