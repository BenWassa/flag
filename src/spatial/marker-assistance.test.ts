...
describe('Spatial marker / practical-touch parity (#200)', () => {
  it('exposes exactly the source-derived anchor used by assisted resolution', () => {
    ...
    // 23 px from the visible marker centre: near the edge of the 24 px
    // practical radius, over uncontested water rather than the tiny polygon.
  ...
  it('retires marker eligibility at the exact screen-scale threshold used by assistance', () => {
    ...
    // At this scale the 0.2 degree country is 200 px wide. It is ordinary,
    // aimable geography, so neither a practical envelope nor a visible marker
    // should remain.
