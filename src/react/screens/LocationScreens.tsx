...
export function LocationResultsScreen({ asset, result }: { asset: MapRegionAsset; result: MapSessionResult }) {
  ...
  return <main className="page page--map-results">
    ...
    <section className={`map-result-summary${perfect ? ' map-result-summary--perfect' : ''}`} aria-labelledby="map-result-heading"><h1>{result.firstTryCorrect} of {result.total} first try</h1><p>{result.missedCountryIds.length === 0 ? 'Clean round. Every location was right immediately.' : `${result.missedCountryIds.length} ${result.missedCountryIds.length === 1 ? 'location needs' : 'locations need'} another pass.`}</p>{perfect ? <span className="result-score__badge">Perfect round</span> : null}<div className="map-result-breakdown" aria-label={`${result.session.mode === 'learn' ? 'Learn' : 'Play'} round breakdown`}>{result.session.mode === 'learn' ? <><span><i className="map-swatch map-swatch--first" aria-hidden="true" /><strong>{result.firstTryCorrect}</strong> first try</span><span><i className="map-swatch map-swatch--one" aria-hidden="true" /><strong>{counts.one}</strong> 1 miss</span><span><i className="map-swatch map-swatch--two" aria-hidden="true" /><strong>{counts.two}</strong> 2 misses</span><span><i className="map-swatch map-swatch--reveal" aria-hidden="true" /><strong>{counts.revealed}</strong> revealed</span></> : <><span><strong>{result.firstTryCorrect}</strong> correct</span><span><strong>{counts.incorrect}</strong> missed</span></>}</div></section>
