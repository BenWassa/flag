 ...
 * geography reaches durable routes, that history still works through the shell,
 * that the stage yields where the contract says it must, and that a device
 * without WebGL still gets a complete Atlas.
 *
...
test.describe('persistent spatial shell', () => {
  ...
  test('native Back and Forward walk the same spatial hierarchy', async ({ page }) => {
    ...
    await scopeName(page, 'Africa').click();
    await expect(page).toHaveURL(/#\/flags\/africa$/);
    await scopeName(page, 'West Africa').click();
...
test.describe('geography routing', () => {
  test('tapping geography and its projected name both reach a durable continent route', async ({ page }) => {
    await openSpatial(page, '/flags');
    ...
    await scopeName(page, label).click();
...
test.describe('accessibility and resilience', () => {
  test('the canvas is not an accessibility surface and projected geography provides every area control', async ({ page }) => {
    await openSpatial(page, '/flags/africa');
    ...
    for (const region of ['North Africa', 'West Africa', 'Central Africa', 'East Africa', 'Southern Africa']) {
      await expect(scopeName(page, region)).toBeAttached();
    }
    await scopeName(page, 'East Africa').focus();
