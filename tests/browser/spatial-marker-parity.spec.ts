...
test.describe('tiny-country Spatial marker parity (#200)', () => {
  ...
  test('single-pointer drag ownership cannot resolve a marker as a tap', async ({ page }) => {
    const markerCase = CENTER_CASES.find((item) => item.id === 'TUV')!;
    const { point } = await openCase(page, markerCase);
    const surface = page.locator(SURFACE);

    await surface.dispatchEvent('pointerdown', {
      pointerId: 21, pointerType: 'touch', button: 0, buttons: 1,
      clientX: point.x, clientY: point.y, isPrimary: true,
    });
    await surface.dispatchEvent('pointermove', {
      pointerId: 21, pointerType: 'touch', buttons: 1,
      clientX: point.x + 9, clientY: point.y, isPrimary: true,
    });
    await surface.dispatchEvent('pointermove', {
      pointerId: 21, pointerType: 'touch', buttons: 1,
    ...
    await surface.dispatchEvent('pointerup', {
      pointerId: 21, pointerType: 'touch', button: 0, buttons: 0,
      clientX: point.x + 30, clientY: point.y, isPrimary: true,
    });
  ...
  test('pinch ownership cannot resolve a marker as a tap', async ({ page }) => {
    const markerCase = CENTER_CASES.find((item) => item.id === 'TUV')!;
    const { point } = await openCase(page, markerCase);
    const surface = page.locator(SURFACE);
