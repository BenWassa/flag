...
function cleanProjectedPath(path, minimumRingArea = MINIMUM_PROJECTED_RING_AREA) {
  ...
  for (const ring of rings) {
    ...
    if (area < minimumRingArea) continue;
...
for (const [id, sourceName, longitudeRanges, excludedCountries = []] of CONTINENTS) {
  ...
  const path = fitCleanPath(cleanProjectedPath(
    geoPath(projection).digits(1)(outline) ?? '',
    MINIMUM_RING_AREA_BY_CONTINENT[id] ?? MINIMUM_PROJECTED_RING_AREA,
  ));
...
const output = `// Generated by scripts/generate-continent-icons.mjs from the pinned Natural Earth source.\n`
  ...
  + `  if (!path) return '';\n`
  + `  return \`<svg class="continent-icon" data-continent="\${id}" viewBox="0 0 ${SIZE} ${SIZE}" aria-hidden="true" focusable="false" fill="currentColor"><path d="\${path}"/></svg>\`;\n`
  + `}\n`;

await writeFile(OUTPUT_PATH, output);
console.log(`Generated ${CONTINENTS.length} continent silhouette icons from ${sourceUrl}.`);
