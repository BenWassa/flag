...
// The supported continent row is one deliberate navigation target, while an
// unshipped continent stays inert: named, but not a control.
assert.equal(
  (locationsIndex.match(/data-action="open-scope"/g) ?? []).length,
  1,
  'Locations exposes one supported continent navigation row.',
);
assert.equal(
